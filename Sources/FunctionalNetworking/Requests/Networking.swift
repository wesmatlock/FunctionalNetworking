...
public func requestSyncR(_ request: URLRequest?) -> IO<Result<Data, Error>> {
  IO(deferred: requestAsyncR(request))
}

public func requestAsyncR(_ request: URLRequest?) -> Deferred<Result<Data, Error>> {
  requestAsyncE(request).map(Result.init(either:))
}

public func requestSyncE(_ request: URLRequest?) -> IO<Either<Error, Data>> {
  IO(deferred: requestAsyncE(request))
}

public func requestAsyncE(_ request: URLRequest?) -> Deferred<Either<Error, Data>> {
...
public func deferredDataTask(request: URLRequest?) -> Deferred<Either<Error, (Data, URLResponse)>> {
  ...
  var deferred = Deferred<Either<Error, (Data, URLResponse)>> { callback in
    ...
    guard let request = request
      ...
      callback(.left(NetworkRequestError.invalidRequest))
      return
    }
