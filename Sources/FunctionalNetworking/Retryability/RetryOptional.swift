...
public func retry<A, B>(_ f: @escaping (A) -> B?, retries: Int, debounce: Debounce) -> (A) -> B? {
  retry (f)(debounce)(retries)
}

public func retry<A, B>(_ f: @escaping (A) -> B?) -> (Debounce) -> (Int) -> (A) -> B? {

  func retry(value: A, result: B?, currentRun: Int, debounce: Debounce) -> B? {
