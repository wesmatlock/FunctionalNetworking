...
public func retry<A, B>(_ f: @escaping (A) -> Reader<A, Result<B, Error>>, retries: Int, debounce: Debounce)
-> (A) -> Reader<A, Result<B, Error>> {
  retry(f)(debounce)(retries)
}

public func retry<A, B>(_ f: @escaping (A) -> Reader<A, Result<B, Error>>) -> (Debounce) -> (Int) -> (A)
-> Reader<A, Result<B, Error>> {

  func retry(value: A, result: Reader<A, Result<B, Error>>, currentRun: Int, debounce: Debounce)
  -> Reader<A, Result<B, Error>> {
